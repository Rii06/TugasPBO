...
public class LaptopViewFrame extends JFrame{
    ...
    public LaptopViewFrame(){
        ...
        cariButton.addActionListener(e -> {
            ...
            String searchSQL = "SELECT laptop.id, merk.merk, laptop.nama, class.jenis_laptop " +
                    "FROM ((laptop INNER JOIN merk ON laptop.id_merk = merk.id) " +
                    "INNER JOIN class ON laptop.id_class = class.id) " +
                    "WHERE merk like ? OR nama like ? OR jenis_laptop like ? ";
            ...
            try {
                ...
                while (rs.next()){
                    ...
                    row[3] = rs.getString("jenis_laptop");
                    dtm.addRow(row);
                }
            } catch (SQLException ex) {
                throw new RuntimeException(ex);
    ...
    public void isiTable(){
        String selectSQL = "SELECT laptop.id, merk.merk, laptop.nama, class.jenis_laptop " +
                "FROM ((laptop INNER JOIN merk ON laptop.id_merk = merk.id) " +
                "INNER JOIN class ON laptop.id_class = class.id)";
        Connection c = Koneksi.getConnection();
        try {
            ...
            String header[] = {"Id","Merk","Nama Laptop","Jenis"};
